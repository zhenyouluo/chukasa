...
//        long[] infoReadData = readDTS(chukasaModel.getReadBytes(), chukasaModel.getSeqTs());
    ...
    long[] readPCR(long readByteInput, int seqTsInput) {
        ...
        final double segmentedTsDuration = (double) chukasaModel.getHlsConfiguration().getDuration();

        try {
            final FileInputStream fileInputStream;
            if(!chukasaModel.getChukasaSettings().getStreamingType().equals(StreamingType.OKKAKE)) {
                fileInputStream = new FileInputStream(chukasaModel.getSystemConfiguration().getTemporaryPath() + FILE_SEPARATOR + STREAM_FILE_NAME_PREFIX + chukasaModel.getChukasaSettings().getVideoBitrate() + STREAM_FILE_EXTENSION);
            }else{
                fileInputStream = new FileInputStream(chukasaModel.getSystemConfiguration().getFilePath() + FILE_SEPARATOR + chukasaModel.getChukasaSettings().getFileName());
            }

            final BufferedInputStream bufferedInputStream = new BufferedInputStream(fileInputStream);
            bufferedInputStream.skip(readByte);
                ...
                if (flagCreateFile) {
                    ...
                    if(!chukasaModel.getChukasaSettings().getStreamingType().equals(StreamingType.OKKAKE)) {
                        ...
                        if (chukasaModel.getChukasaSettings().isCanEncrypt()) {
                            ...
                            File tempSegDir = new File(chukasaModel.getTempEncPath());
                            tempSegDir.mkdirs();
                            f = new FileOutputStream(chukasaModel.getTempEncPath() + FILE_SEPARATOR + STREAM_FILE_NAME_PREFIX + segmentedTsDuration + STREAM_FILE_EXTENSION);
                            bos = new BufferedOutputStream(f, MPEG2_TS_PACKET_LENGTH );
                            log.info("Begin Segmentation of seqTs : {}", sequenceMediaSegment);
                            flagCreateFile = false;

                        } else {

                            f = new FileOutputStream(chukasaModel.getStreamPath() + FILE_SEPARATOR + STREAM_FILE_NAME_PREFIX + sequenceMediaSegment + STREAM_FILE_EXTENSION);
                            bos = new BufferedOutputStream(f, MPEG2_TS_PACKET_LENGTH );
                            log.info("Begin Segmentation of seqTs : {}", sequenceMediaSegment);
                            flagCreateFile = false;
...
//                        if (chukasaModel.getChukasaSettings().isEncrypted()) {
                        ...
                        File tempSegDir = new File(chukasaModel.getTempEncPath());
                        tempSegDir.mkdirs();
                        f = new FileOutputStream(chukasaModel.getTempEncPath() + FILE_SEPARATOR + STREAM_FILE_NAME_PREFIX + sequenceMediaSegment + STREAM_FILE_EXTENSION);
                        bos = new BufferedOutputStream(f, MPEG2_TS_PACKET_LENGTH );
                        log.info("Begin Segmentation of seqTs : {}", sequenceMediaSegment);
                        flagCreateFile = false;
...
//                        }
                ...
                if(canSegment){
                    ...
                    chukasaModel.setSeqTsEnc(sequenceMediaSegment);
                    chukasaModel = chukasaModelManagementComponent.update(adaptiveBitrateStreaming, chukasaModel);
                    ...
                    if(!chukasaModel.getChukasaSettings().getStreamingType().equals(StreamingType.OKKAKE)) {
                        ...
                        chukasaModel.setSequenceMediaSegment(sequenceMediaSegment);
                        chukasaModel = chukasaModelManagementComponent.update(adaptiveBitrateStreaming, chukasaModel);
            ...
            if (chukasaModel.isFlagLastTs()) {
                //chukasaModel.setSeqTsLast(seqTs - 1);
                chukasaModel.setSeqTsLast(sequenceMediaSegment);
            ...
            long[] ret = new long[2];
            ret[0] = (MPEG2_TS_PACKET_LENGTH  * (countPacket)) + readByte; // MPEG2-TS byte length ever read
            ret[1] = sequenceMediaSegment; // MPEG2-TS sequences ever segmented
            return ret;
